...
local function md5(content)
    local stdout = luci.sys.exec('echo "%s" | md5sum | cut -d " "  -f1' % urlEncode(content))
...
local function wget(url)
    local stdout = luci.sys.exec('wget-ssl --user-agent="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.3865.90 Safari/537.36" --no-check-certificate -t 3 -T 10 -O- "%s"' % url)
    return trim(stdout)
end
...
local execute = function()
    ...
    do
        ...
        if firstServer then
            luci.sys.call('/etc/init.d/%s restart > /dev/null 2>&1 &' % name) -- 不加&的话日志会出现的更早
        else
            luci.sys.call('/etc/init.d/%s stop > /dev/null 2>&1 &' % name) -- 不加&的话日志会出现的更早
        end
        log('新增节点数量: %d 删除节点数量: %d' % {add, del})
        log('更新成功服务正在启动')
        iret = add
    end
end
