...
function m.commit_handler4(self)
	if self.changed then	-- changes ?
		os.execute("/etc/init.d/shadowsocksr reload &")	-- reload configuration
	end
end
...
function s.parse(self, ...)
	...
	if created then
		...
		luci.http.redirect(disp.build_url("admin", "services", "shadowsocksr", "dnscrypt-proxy"))
	end
end
