...
local ds = require "luci.model.dnscrypt".init()
...
s.extedit = luci.dispatcher.build_url("admin/services/shadowsocksr/dnscrypt-resolvers", cfg, "%s")
...
m.on_before_commit = function (self)
	...
	self.redirect = luci.dispatcher.build_url("admin/services/shadowsocksr/dnscrypt-resolvers", self.config)
	...
	if self:get(section, "urls") and not on_remove then
	 	if not c:get_first("ucitrack", self.config, "exec", nil) then
	 		...
	 		luci.util.exec("echo uci_track register handle: %s >>/tmp/ssrplus.log" % sec)
	 	end
	else
	 	if c:get_first("ucitrack", self.config, "exec", nil) == ("%s %s" %{exec, self.config}) then
	  	c:delete_all("ucitrack", self.config, function(section) return true end)
	  	c:save("ucitrack"); c:commit("ucitrack");
	 	 luci.util.exec("echo uci_track remove handle: %s >>/tmp/ssrplus.log" % sec)
	 	end
	 	self.redirect = luci.dispatcher.build_url("admin/services/shadowsocksr/dnscrypt-resolvers")
	end
	c:unload("ucitrack")
end
...
if cfg then
	...
	luci.http.redirect(luci.dispatcher.build_url("admin/services/shadowsocksr/dnscrypt-resolvers"))
	return
end
